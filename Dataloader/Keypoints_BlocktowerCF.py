...
class blocktowerCF_Keypoints(Dataset):
    def __init__(self, mode="train", model="derendering", n_kpts=4):
        super(blocktowerCF_Keypoints, self).__init__()

        self.dataloc = f"blocktower/{n_kpts}/{model}/"
        assert os.path.isdir(self.dataloc)

        with open(f"Datasets/blocktowerCF_4_{mode}", "r") as file:
