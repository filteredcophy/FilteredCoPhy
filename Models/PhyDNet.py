...
class EncoderRNN(torch.nn.Module):
    ...
    def forward(self, input, first_timestep=False, decoding=False):
        ...
        out_phys = torch.tanh(self.decoder_D(decoded_Dp))  # partial reconstructions for vizualization
        out_conv = torch.tanh(self.decoder_D(decoded_Dr))
    ...
    def reconstruct(self, input):
        ...
        input_phys = self.encoder_Ep(input)
        input_conv = self.encoder_Er(input)
        decoded_Dp = self.decoder_Dp(input_phys)
        decoded_Dr = self.decoder_Dr(input_conv)
        concat = decoded_Dp + decoded_Dr
        output_image = torch.tanh(self.decoder_D(concat))
        return output_image
